/**
 * The 747's stowed spoilers as the chase camera sees them: from behind and
 * above, at a known distance, with each spoiler mesh's PROJECTED bounding box
 * written beside the PNG -- so the crop is placed from the geometry, never by
 * searching the image for a colour.
 *
 *   npx tsx scripts/spoiler-stowed-frames.mts <outDir> <url> <expectTree> <distanceM> <label> [elevationDeg] [azimuthDeg]
 *
 * `azimuthDeg` turns the camera round the aeroplane in its own frame: 0 is
 * abeam to starboard, 90 dead astern, 150 behind and to starboard (the
 * default). `elevationDeg` (default 17, the chase camera's own angle at its
 * 112 m standoff) lifts it above the wing plane. The camera aims at the centre
 * of the starboard spoiler meshes and is re-parked every frame. Which checkout
 * the dev server serves is proved from the listening process's working
 * directory before anything is captured.
 */
...
const elevation = Number(elevationRaw ?? 17);
const azimuth = Number(azimuthRaw ?? 150);
...
const SPOILERS = /^starboard-airliner-(ground|flight)-spoilers-surface$/;
...
const parked = await page.evaluate(async ({ d, el, az, pattern }: { d: number; el: number; az: number; pattern: string }) => {
  ...
  const spoilers = scene.meshes.filter((m) => matcher.test(m.name));
  const shell = scene.meshes.find((m) => m.name === "airliner-fuselage-shell");
  if (spoilers.length !== 2 || !shell) return `expected 2 starboard spoiler meshes and the shell, found ${spoilers.length}`;
  ...
  scene.onBeforeRenderObservable.add(() => {
    ...
    // Aim: the centre of the starboard spoilers' world boxes.
    let sx = 0, sy = 0, sz = 0, n = 0;
    for (const mesh of spoilers) {
  ...
  return "parked";
}, { d: distance, el: elevation, az: azimuth, pattern: SPOILERS.source });
...
const stem = `${outDir}/${label}-spoilers-${distance}m-el${elevation}-az${azimuth}`;
await page.screenshot({ path: `${stem}.png`, type: "png" });

// Each starboard spoiler mesh's box, projected to PNG pixels.
...
}, { pattern: SPOILERS.source, width: WIDTH * 2, height: HEIGHT * 2 });

writeFileSync(`${stem}.json`, JSON.stringify({ label, distance, elevation, azimuth, png: `${stem}.png`, pageErrors, boxes }, null, 2));
console.log(`wrote ${stem}.png; spoiler boxes: ${boxes.map((b) => `${b.name} ${Math.round(b.x1 - b.x0)}x${Math.round(b.y1 - b.y0)}`).join(", ")}; page errors: ${pageErrors.length}`);
await browser.close();
