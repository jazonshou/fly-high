...
Pure. No Babylon import, no canvas, no DOM, no `fs`. **Every Node test runs
...
That is 2.93 cm per texel along the body and ~4 cm around the section at the
cabin — near enough isotropic. A one-to-two texel cheatline edge is 3-6 cm
against the ~0.8 m the vertex-colour band manages today, a 15-25x improvement.

Cost: 4 MB, ~5.6 MB with the mip chain. If that is judged too much, 1024 x 512
is 2.8 MB and still beats today by 13x; the generator must not hardcode either,
...
23 texels low at the nose: a band that visibly droops forward, which is the
defect this texture exists to remove.
...
- **Cheatline**, navy, linear RGB `[0.011, 0.042, 0.147]` (sRGB ~ 27,58,107).
  **BELOW the main-deck window line**: main-deck panes sit at y = 0.2 and are
  ...
  range: dies out by x = -24.5 aft and x = +30.5 forward, as the vertex band
  does now.
...
- **Spoiler, flap and aileron panel lines** on the wing surfaces.
- **Nacelle lines** and the **wing-root tone**.
...
8. **Nothing bleeds to the seam**: column u=0 and the last column are white at
   the crown rows, so the wrap is clean.

The mip chain is asserted where the texture is uploaded, not here — this module
returns pixels and knows nothing about Babylon.
