...
| D-9 | A3 | The decision-log row's "trainer suites pass unchanged" was imprecise: `tests/sim.rebuild.test.ts` flies the TRAINER through the retention path and its settle tolerances were re-pinned (3.5°→10° nose-down, 4°→8° nose-up, with new directional asserts) for the A3 rate-led capture. The behavior change was deliberate A3 scope; the claim under-reported which suites it reached. Caught by the Phase 5 close agent's independent verification at `6a5b29e`. | Honesty of the record; the re-pin itself is correct and the directional asserts are stronger than the old tolerance-only form. | 2026-08-26 |
