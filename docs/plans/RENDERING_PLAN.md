...
| 6-11 | `quality-tiers-v2` — four tiers on measured numbers, asserted in CI | 3.0 | all | See §5.3. Measure default-eroded cold startup to first ready separately from steady `perf:capture`; record the startup-stage split and fail on timeout or console error. Expect the first real capture to move several rows by 30–50%. |
| 6-12 | Documentation truth pass — `README.md:54-60`, `docs/PERFORMANCE.md:41,61-76`, pinned by tests that read the profile table and startup result | 1.0 | 6-11 | Reconcile both the tier table and the actual startup critical path. Renderer creation is intentionally outside steady capture, so the cold-load result must be sourced independently. **This is why the regressions went unnoticed.** |

**Exit criteria.** All four tiers pass `assertWithinBudget()` at three viewport sizes. Balanced holds 60 fps on the reference M2 Pro with erosion, textured terrain, flowing rivers and 3 km of vegetation, measured by `perf:capture`, not by impression. A separate reference-machine cold default-eroded launch reaches first ready within its committed deadline with no startup or console error. README and PERFORMANCE.md assertions pass.
...
**Amendment, 2026-09-13 (user decision): the far field must exist.** Flight testing found that trees "mostly don't exist from a distance" and "pop into place" on approach. Cause: the impostor band inherited the geometry share's floor (0.045 at Balanced, ~3.5 stems/ha beyond ~700 m), so a forest past the mid band read as scattered dots on the terrain's flat canopy albedo, and a stem's first appearance was its CELL's rebuild on approach. The user asked for distant trees to exist — "maybe with 2d" — and for the transition to be smooth, without excessive frame cost. The rendered-density law therefore carries a second floor, `impostorFloorShare` (0.20 / 0.30 / 0.35 / 0.40, a new row in the tier table): the share of the near cap drawn in SOME representation beyond the near radius. Between the geometry share and this floor every stem is a 2D octahedral impostor — inside the mid band from the crossover `near / √floor` (~274 m at Balanced) as well as beyond it — and the GPU thins per stem against the live range, so a stem changes representation at its own threshold instead of appearing from nothing. **This raises a count row (lever 1) without a fidelity row moving, which the ratchet forbids; it is booked here as the user's own reversal of that trade for the far field specifically.** The 2D representation is the cheapest one the renderer has (measured on the M2 Pro at Balanced: `forest-line-highsun` 101.5 → 101.1 fps with the fill, once a key-clamp defect that briefly drew the whole authored field was fixed), and the woody triangle ceilings moved with the law (`renderedDensity.ts`, R-22). The trade-off rule otherwise stands: near-field count still yields before fidelity when a budget binds.
