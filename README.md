...
- Fixed 120 Hz simulation in a dedicated Web Worker, decoupled from render FPS, with a 60 Hz state stream presented on snapshot simulation time. A smoothed worker-clock offset absorbs arrival jitter, sampling never moves backwards, and a frame may coast for at most 50 ms from linear and angular velocity instead of snapping when delivery is late. Exterior camera position, aim, up-vector and FOV ease as one rig; cockpit presentation remains exact.
...
- `src/workers/` — independent simulation and hydrology workers, protocols, and bounded schedulers. (The terrain worker is gone: terrain pages are a compute dispatch.)
...
The renderer monitors rolling 120-frame CPU and, when available, full-frame GPU p95 timings and adjusts internal resolution slowly between the profile ceiling and a 0.75 floor. When resolution stops buying frame time the governor latches and sheds work instead, along two disjoint ladders — CPU-cost levers and GPU-cost levers — so a GPU-bound window is never answered with a CPU-side saving. Device pixel ratio is capped per tier (1 / 1.5 / 2 / 2), MSAA runs on the offscreen beauty target at per-tier sample counts (1 / 2 / 2 / 4), and FXAA after half-float ACES color processing serves only the no-MSAA tier. Diagnostics expose start-to-start frame-interval p95 alongside CPU/GPU time and p95, the worst and 99.9th-percentile frame times and a hitch count (frames slower than twice the tier target) over a rolling 600-frame window, the active governor and the lever it moved last, draw calls, triangles, resident pages, visible instances, animals, rivers/lakes, render scale, cloud steps, ocean FFT topology, and adapter name. The asynchronous GPU counter has no submitted-frame identifier, so the renderer deliberately does not manufacture a per-frame “present wait” by subtracting unrelated samples.
