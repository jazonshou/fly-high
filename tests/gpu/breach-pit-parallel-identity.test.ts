 ...
 * surface, run the parallel carve chunk by chunk as the DAG does (indirect,
 * one workgroup per pit, each chunk from its own arg set, some chunks sharing
 * a frame and some not) and read the surface and the receivers; then re-run
      ...
      try {
        for (const [level, x, z] of PAGES) {
          ...
          for (let chunk = 0; chunk < chunks; chunk += 1) {
            let attempt = 0;
            while (!shaders.breachPit.dispatchIndirect(buffers.pitArgs, chunk * 16)) {
              if ((attempt += 1) > 200) throw new Error("carve pipeline never ready");
              await nextFrame();
            }
