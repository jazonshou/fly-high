...
function stagedProducer(options: {
  ...
  const shader = (name: string) => ({
    name,
    dispatch: (x: number) => {
      if (name === "breachPit" && carveRefusals > 0) { carveRefusals -= 1; calls.push(`not ready ${name}`); return false; }
      calls.push(`dispatch ${name} ${x}`);
      return true;
    },
    dispatchWhenReady: async (x: number) => { calls.push(`dispatchWhenReady ${name} ${x}`); },
    dispatchIndirect: () => { calls.push(`INDIRECT ${name}`); return true; },
  });
  const buffers = {
    pitArgs: {
      label: "pitArgs",
...
describe("breach as passes: the stage machine", () => {
  it("resets the count, lists, sizes, reads the count, then carves a chunk at a time from each chunk's own arg set", async () => {
    ...
    expect(calls.filter((call) => (call.startsWith("dispatch breachPit") || call.startsWith("INDIRECT")))).toEqual([]);
    releaseReads();
    await settle();
  ...
  it("waits for a cold carve pipeline and records each chunk at its size, in order", async () => {
    const { producer, calls, job } = stagedProducer({ carveReadyAfter: 1 });
    Object.assign(job, { breachDirectDone: true, breachArgsDone: true, breachChunks: 2, breachListed: 200 });
    await producer.pump(2);
    expect(calls).toEqual([
      "not ready breachPit",
      "dispatchWhenReady breachPit 128",
      "dispatch breachPit 72",
      "readback",
    ]);
    expect(job).toMatchObject({ breachChunksDone: 2, stage: "readback" });
  });
...
describe("breach as passes: a page whose pits overflow the list fails loudly", () => {
  ...
  it("fails an overfull page at the count read, before any chunk is carved, and counts it", async () => {
    ...
    expect(calls.filter((call) => (call.startsWith("dispatch breachPit") || call.startsWith("INDIRECT")) || call === "readback")).toEqual([]);
  });
  ...
  it("re-reads a count that came back as zeros, and fails the page rather than carve nothing on a second", async () => {
    ...
    expect(twice.calls.filter((call) => (call.startsWith("dispatch breachPit") || call.startsWith("INDIRECT")) || call === "readback")).toEqual([]);
  });
...
describe("breach as passes: the WGSL", () => {
  ...
  it("dispatches each chunk directly at its listed size, never indirect, and never the serial control", () => {
    expect(source).toContain(
      'shaders.breachPit, "breachPit", Math.min(BREACH_PIT_CHUNK_PITS, job.breachListed - first), 1, 1, job);');
    expect(source).toContain("job.breachListed = Math.min(pits, BREACH_PIT_LIST_CAPACITY);");
    expect(stripComments(source)).not.toContain("dispatchIndirect");
    expect(source).toContain("{ computeSource: breachPitWgsl() },");
    expect(source).not.toContain("{ computeSource: breachPitSerialWgsl() },");
  });

  it("creates the count buffer writable and readable back, and never usable as INDIRECT", () => {
    ...
    expect(source).toContain("const PIT_ARGS_CREATION_FLAGS = 3;");
    expect(source).toContain(
